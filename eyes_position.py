...
with mp_face_mesh.FaceMesh(max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5,
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            right_iris_pos, ratio_r = iris_position(center_right, mesh_points[R_H_RIGHT], mesh_points[R_H_LEFT][0])
            left_iris_pos, ratio_l = iris_position(center_left, mesh_points[L_H_LEFT], mesh_points[L_H_RIGHT][0])

            # print('Left Eye: ' + left_iris_pos + '|| Right Eye: ' + right_iris_pos)

            cv2.putText(frame, 'Left Iris Position: {} | Right Iris Position {}'.format(left_iris_pos, right_iris_pos),
                        (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
